    ...
    xhr.onreadystatechange = function() {
        ...
        if (xhr.readyState === 4) {
            ...
            if (xhr.status === 200) {
                ...
                // 로드된 페이지가 로그인 페이지인 경우
                if (pageName === 'login') {
                    // 로그인 폼 검증 함수를 실행
                    attachLoginFormValidation();
                } 
                
                // 로드된 페이지가 회원가입 페이지인 경우
                if (pageName === 'sign-up') {
                    // 회원가입 폼 검증 함수를 실행
...
// 해당 코드는 main.html이 로드되지 않는 문제점을 발견하여 코드를 변경합니다.
...
// DOMContentLoaded 이벤트를 활용하여 DOM이 완전히 로드된 후 main.html을 로드합니다.
...
function hideLoginModal() {
    ...
    // 애니메이션이 끝난 후 모달을 화면에서 숨김 및 초기화 작업
    setTimeout(function() {
        modal.style.display = 'none';
        modalContent.style.animation = 'none'; // 애니메이션 초기화
    }, 500); // 애니메이션 지속 시간과 일치하도록 설정 (0.5초)
